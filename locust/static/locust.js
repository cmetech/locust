...
var responseTimeChart = new LocustLineChart(
  ...
  'Response Times (ms)',
  [
    (percentile1 * 100).toString() + 'th percentile',
    (percentile2 * 100).toString() + 'th percentile',
  ],
  'ms'
);
...
function updateStats() {
  $.get('./stats/requests', function (report) {
    ...
    try {
      ...
      stats_history['response_time_percentile_1'].push({
        value: report.current_response_time_percentile_1,
        users: report.user_count,
      });
      stats_history['response_time_percentile_2'].push({
        value: report.current_response_time_percentile_2,
        users: report.user_count,
      });
      update_stats_charts();
    } catch (i) {
